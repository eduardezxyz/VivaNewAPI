    ...
    public class ServiceUserController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreatePayApp([FromBody] PayAppsVw model) //NOT working 
        {
            ...
            try
            {
                ...
                var spv = await _context.PayAppsVws.FirstOrDefaultAsync(l => l.PayAppId == databaseModel.PayAppId);
        ...
        public async Task<IActionResult> UpdatePayApp([FromBody] PayAppsVw model) //same as CREATE 
        {
            ...
            if (model.StatusId == 3 && previousStatusId != 3)
            {
                try
                {
                    ...
                    {
                        await _emailService.sendAdminPayAppApproved(User.Identity.GetUserId(), spv.ProjectId, spv.SubcontractorId, spv.PayAppId);
                    }
                }
                catch (Exception ex)
                {
        ...
        {
            var data = _context.PayAppsVws.Where(p => p.CreatedByUser == User.Identity.Name);
            return Ok(data);
        }
        ...
        {
            var payApp = _context.PayAppsVws
        ...
        {
            var payAppIds = _context.PayAppsVws
                .Where(p => p.CreatedByUser == User.Identity.Name)
                .Select(p => p.PayAppId);

            var payAppHistories = _context.PayAppHistoryVws
        ...
        {
            bool isCreatorOfPayApp = _context.PayAppsVws
